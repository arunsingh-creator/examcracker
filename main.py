...
now = datetime.now()
date_time = now.strftime("Netflix Accounts Generated At [%m_%d_%Y %H_%M_%S]")



email_domain = "@weyeyoy526@wiicheat.com"# lol
...
def generateNetflixAccount(start, stop):
     ...
     for i in range(start, stop):
         ...
         url = "https://netflix.com/signup/public/v1/account"
         ...
         headers = {
            ...
            "Accept": "*/*",
            "User-Agent": "Netflix/8.5.7iOS/13.5.1(iPhone12, 8)",
         ...
         if result['status'] == 1:
           ...
           window["Verification_Email"].print(f"https://temp-mail.org/en/{email}")
           to_verify.append(f"https://temp-mail.org/en/{email}")
...
Layout = [[sg.Image(filename="Icons/netflix (1).png", size=(50, 50)),sg.Text("Netflix Account Generator",font=("",25))],
          [sg.Text("Developed by Fluro J",font=("",15))],
...
window = sg.Window('Netflix Account Generator' , Layout,element_justification='center')
...
while True:
    ...
    if event == "Generate_Accounts":
        ...
        for n in range(0, no_of_bins, int(no_of_bins/2)):
            ...
            Thread(target=generateNetflixAccount, args=(n, stop),daemon=True).start()
